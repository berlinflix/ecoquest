...
class _SocialScreenState extends State<SocialScreen> {
  ...
  Widget build(BuildContext context) {
    if (_currentUid == null) {
      return const Center(
        child: Text("Please log in to view the leaderboard."),
      );
    }
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.white.withOpacity(
                    0.0,
                  ), // Removing the white background to let the pixel art bleed consistently
                ),
                child: Row(
  ...
  Widget _buildUserHighlightCard(
    String firstName,
    String lastName,
    int level,
    int exp,
        ...
        return Column(
          children: List.generate(docs.length, (index) {
            ...
            if (index == 0) {
              ...
              iconToUse = Icons.surfing;
            } else if (index == 1) {
              iconBgColor = Colors.blue.shade100;
              iconToUse = Icons.hiking;
            } else if (index == 2) {
              iconBgColor = const Color(0xFF25F4AF).withOpacity(0.1);
              iconToUse = Icons.sailing;
            } else {
              iconBgColor = Colors.yellow.shade100;
              iconToUse = Icons.kayaking;
            }
              ...
              child: _buildFriendTile(
                ...
                onTap: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (_) =>
                          FriendProfileScreen(userId: uid, userData: data),
                    ),
                  );
                },
              ),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Request sent to @$targetUsername!')),
        );
      }
    } catch (e) {
      if (mounted) {
        ...
        children: [
          ...
          Expanded(
            child: _isSearching
                ...
                ? Center(
                    child: Text(
                      "Search for an Eco Quest handle\nor sync your contacts.",
                    ...
                    builder: (context, currentUserSnapshot) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          bool isPending =
                              mySent.contains(uid) || myPending.contains(uid);
      ...
      if (status.isGranted) {
        ...
        if (normalizedContacts.isEmpty) {
          if (mounted)
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('No valid phone numbers found in contacts.'),
              ),
            );
          setState(() => _isSearching = false);
          return;
        ...
        for (var doc in allUsersSnapshot.docs) {
          ...
          if (userMobile != null && userMobile.isNotEmpty) {
            ...
            String userLast10 = userDigits.length >= 10
                ? userDigits.substring(userDigits.length - 10)
                : userDigits;

            if (userLast10.length > 5 &&
                normalizedContacts.contains(userLast10)) {
              matches.add(doc);
            }
          }
        }
        ...
        if (matches.isEmpty && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'None of your contacts are currently playing Eco Quest.',
              ),
            ),
          );
        }
        ...
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Permission to read contacts denied.'),
            ),
          );
        setState(() => _isSearching = false);
      }
